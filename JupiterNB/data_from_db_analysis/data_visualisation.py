...
def plot_over_image(img, x, y, **kwargs):
    ...
    plt.imshow(img)
    plt.plot(x * scaleX, y * scaleY, **kwargs)o
...
def plot_rssi_map(df, figsize=(15, 10), filter_func=None):
    min_time = df.Timestamp.min()
    unique_minors = sorted(df.Minor.unique())
    fig = plt.figure(figsize=figsize)
    for i, minor in enumerate(unique_minors):
...
def plot_session(df, filter_func=lambda x: x):
    plt.title("Route")
    plt.scatter(df.X, -df.Y)
    plot_rssi_data(df, filter_func=filter_func)
    plot_rssi_map(df, filter_func=filter_func)
