...
def build_alternative_rssi_map(session_names=[]):
    sessions = list(CaptureSession.select()
                .where(CaptureSession.name << session_names))
    ...
    for session in sessions:
        processed_rssi = list(session.rssi_values)
        processed_positions = list(session.positions)

        i = 0
        for current in range(1, len(processed_positions), 2):
            end_t = processed_positions[current].timestamp

            x = processed_positions[current].x
            y = processed_positions[current].y

            rssi = defaultdict(list)

            while i < len(processed_rssi) and processed_rssi[i].timestamp <= end_t:
                if processed_rssi[i].beacon_uuid != 'b9407f30f5f8466eaff925556b57fe6d':
                    i += 1
                    continue
                rssi[processed_rssi[i].beacon_minor].append(processed_rssi[i].rssi)
                i += 1

            coords.append((x, y))

            pack = []
            minors = sorted(rssi.keys())
            for minor in minors:
                pack.append(np.mean(rssi[minor]))
                pack.append(np.std(rssi[minor]))
            packs.append(pack)
